...
async def convert_html_to_react(request: HtmlToReactRequest):
    ...
    try:
        ...
        react_content = react_content.replace("```tsx", "").replace("```", "").replace("typescript", "").replace("javascript", "").strip()
        ...
        return HtmlToReactResponse(
            success=True, 
            react_component=filtered_react_content,
            message="React component generated successfully"
        )
        
    except ResourceExhausted:
        raise HTTPException(status_code=429, detail="Đã hết lượt AI miễn phí.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to generate React code: {str(e)}")
...
async def edit_html_with_images_endpoint(
    ...
    try:
        ...
        # Gom các phần tử lại để gửi cho Gemini (bao gồm ảnh và text)
        content_parts = [
            {"mime_type": current_ui_image.content_type, "data": current_ui_content},
            f"CURRENT HTML TO EDIT:\n{html}\n\nTEXT PROMPT FOR CHANGES:\n{prompt}"
        ]
        
        # Nếu có ảnh sketch thì chèn thêm vào
        if sketch_image:
            sketch_content = await sketch_image.read()
            content_parts.insert(1, {"mime_type": sketch_image.content_type, "data": sketch_content})
        ...
        edited_html = edited_html.replace("```html", "").replace("```", "").strip()
        ...
        raise HTTPException(status_code=429, detail="Đã hết lượt AI miễn phí. Vui lòng thử lại sau!")
    except Exception as e:
        print(f"Lỗi Edit HTML: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to edit HTML: {str(e)}")
