...
Your task is to convert a static HTML/Tailwind snippet (derived from a PowerPoint slide) into a clean, functional, and reusable React functional component.

### KEY OBJECTIVES:

1. **COMPONENT ARCHITECTURE:**
   - Create a single, default export functional component.
   - Use Lucide-react for any icons if identified, or keep original SVG/Img tags.
   - Maintain the `absolute` positioning and specific dimensions provided in the HTML to ensure visual fidelity to the original slide.

2. **DATA DRIVEN DESIGN (PROPS):**
   - Identify text elements, images, and repeating lists.
   - Abstract these into a logical `data` object or `props` so the component can be rendered with different content easily.
   - Use descriptive prop names (e.g., `title`, `subtitle`, `bulletPoints`, `heroImage`).

3. **CLEAN CODE STANDARDS:**
   - Use Tailwind CSS utility classes exclusively.
   - Ensure the component is responsive within its relative container.
   - If there are repetitive elements (like 3-4 similar icons or text boxes), use a `.map()` function to render them from an array.

4. **OUTPUT CONSTRAINTS:**
   - Provide ONLY the React component code.
   - DO NOT include Markdown code blocks (no ```jsx or ```tsx).
   - DO NOT provide explanations, setup instructions, or any introductory text.
   - Ensure the code is ready to be saved as a `.tsx` or `.jsx` file and rendered immediately.

### EXAMPLE STRUCTURE:
(Your output should follow this pattern but with the actual design logic)
const SlideComponent = ({ data }) => {
  return (
    <div className="relative w-[960px] h-[540px] bg-white overflow-hidden">
      {/* Absolute positioned elements here */}
    </div>
  );
};
export default SlideComponent;

Process the input HTML carefully and transform it into a professional React component.
"""